...
if(msg.content === 'peer pressure') {
  ...
  if (msg.content ===  'How many members do I have') {
    msg.channel.send('You have ' + message.guild.memberCount + ' members.');
  }
  if (msg.content === 'How many members do I have?') {
    msg.channel.send('You have ' + message.guild.memberCount + ' members.');
  }
  if (msg.content === 'How many channels do I have'){
    msg.channel.send('You have ' + message.guild.channels.size + ' channels.');
  }
  if (msg.content === 'How many channels do I have?'){
    msg.channel.send('You have ' + message.guild.channel.size + ' channels.');
  }
  if (msg.content === 'apple') {
    msg.channel.send('https://www.apple.com');
  }
...
break;
    ...
    case "setusernick":
      ...
      let unickstaffRoleIDs = message.guild.roles.filter(r => r.name == "Owner" || r.name == "Admin" || r.name == "Admins" || r.name == "Administrator").map(r => r.id);
...
if (unickisStaff) {
  ...
  let usernick = message.content.split(/\s+/g).slice(2).join(" ")
      ...
      message.channel.send('Nickname set to ' + usernick)
    }
    else {
      ...
      break;
    case "setbotnick":
    let bnickstaffRoleIDs = message.guild.roles.filter(r => r.name == "Owner" || r.name == "Admin" || r.name == "Admins" || r.name == "Administrator").map(r => r.id);
...
if (bnickisStaff) {
    ...
    let botnick = message.content.split(' ').slice(1).join(' ')
      ...
      message.channel.send('Nickname of HackerBot set to ' + botnick + ' on the server ' + message.guild.name)
      console.log('Nickname of HackerBot set to ' + botnick + ' on the server ' + message.guild.name)
    }
    else {
...
case "say":
    ...
    case "changelog":
      ...
      .addField('Added >math', 'Have HackerBot do your Math Homework for you')
      .addField('Added >pi', '3.14andabunchofnumbersafterthat')
      .addField('Bot token moved back to config.json','Compatibility errors have been fixed/bypassed')
      .addField('Added >setnick','Get the nickname setting commands')
      .addField('Added >members and >membercount','Get your server\'s Member Count')
      .addField('Added >channels and >channelcount','Get your channel\'s Server Count')
      .addField('Added >servername','Get your server\'s name')
      .addField('Added >serverid','Get your server\'s Guild ID')
      .addField('Removed >logging','Useless from the start')
      .setFooter('Changelog ' + info.version,'https://images-ext-1.discordapp.net/external/Z1krf1ZBQTM3mDkke47xThwIF4GEFG0doRAdKRVHgnY/%3Fsize%3D128/https/cdn.discordapp.com/avatars/342054071060004884/4b83b2aa5970b8d34db71fd986abfe30.jpg')
   		message.channel.sendEmbed(embed)
   	  	break;
     default:
      message.channel.send(" :x: Sorry, but that command is not Hacker approved. :x: ")
      break;
